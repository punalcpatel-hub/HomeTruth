...
type Property = { id:string; address:string; city:string; state:string; zip:string; beds:number|null; baths:number|null; sqft:number|null; year_built:number|null; score:number; summary:string; latitude?:number|null; longitude?:number|null };
...
export default function Home() {
  ...
  useEffect(() => {
    const q = query.trim();
    if (q.length < 5) { setAddressMatches([]); return; }
    const timer = window.setTimeout(async () => {
      setAddressLoading(true);
      try {
        const response = await fetch(`/api/address-search?q=${encodeURIComponent(q)}`);
        const data = await response.json();
        setAddressMatches(Array.isArray(data.matches) ? data.matches : []);
      } catch {
        setAddressMatches([]);
      } finally {
  ...
  async function chooseAddress(match:AddressMatch) {
    ...
    setMessage('Opening property profile...');
    setAddressMatches([]);
    setQuery(`${street}, ${match.city}, ${match.state} ${match.zip}`);
    ...
    if (existing) {
      setSelected(existing as Property);
      setMessage('Existing HomeTruth profile found.');
      return;
    }
    ...
    if (error) {
      const {data:retry} = await supabase
        ...
        .maybeSingle();
      if (retry) {
        setSelected(retry as Property);
        setMessage('Existing HomeTruth profile found.');
      } else setMessage(error.message);
      return;
    }

    const property = created as Property;
    setProperties(prev => [property,...prev]);
    setSelected(property);
    setMessage('New HomeTruth property profile created.');
  }

  async function signOut() {
    await supabase.auth.signOut(); setAuthOpen(false); setMessage('Signed out.');
  }

  async function toggleSave(propertyId:string) {
    if (!user) { setAuthOpen(true); setMessage('Sign in to save homes.'); return; }
    if (saved.has(propertyId)) {
      ...
      if (!error) setSaved(prev=>{const n=new Set(prev);n.delete(propertyId);return n;}); else setMessage(error.message);
    } else {
      const {error} = await supabase.from('saved_properties').insert({user_id:user.id,property_id:propertyId});
      if (!error) setSaved(prev=>new Set(prev).add(propertyId)); else setMessage(error.message);
    }
  }
  ...
  async function submitReview(e:FormEvent<HTMLFormElement>) {
    e.preventDefault();
    if (!selected) return;
    if (!user) { setReviewOpen(false); setAuthOpen(true); setMessage('Sign in before submitting a review.'); return; }
    const form = new FormData(e.currentTarget);
    setMessage('Submitting review...');
    const {error} = await supabase.from('property_reviews').insert({
      property_id:selected.id, reviewer_id:user.id, relationship:String(form.get('relationship')),
      overall_rating:Number(form.get('overall_rating')), noise_rating:Number(form.get('noise_rating')),
      maintenance_rating:Number(form.get('maintenance_rating')), build_quality_rating:Number(form.get('build_quality_rating')),
      parking_rating:Number(form.get('parking_rating')), hoa_rating:Number(form.get('hoa_rating')),
      review_text:String(form.get('review_text')), verification_status:'unverified', moderation_status:'pending'
    });
    if (error) { setMessage(error.message); return; }
    setReviewOpen(false); setMessage('Review submitted. It will appear publicly after moderation.');
  }
